...
with tab2:
    ...
    with col2:
        ...
        if selected_var == 'Índices de Perfusão':
            ...
            perfusion_stats.index = [var_names[col] for col in perfusion_stats.index]
            st.dataframe(perfusion_stats.style.format("{:.2f}"), height=250)
            
        elif selected_var == 'Pressões Arteriais':
            pressure_stats = df[['systolic_mmhg', 'diastolic_mmhg', 'map_mmhg']].describe().T
            pressure_stats.index = [var_names[col] for col in pressure_stats.index]
            st.dataframe(pressure_stats.style.format("{:.2f}"), height=250)
            
        else:
            original_var = [k for k, v in var_names.items() if v == selected_var][0]
            stats = df[original_var].describe().to_frame().T
            stats.index = [selected_var]
        ...
        if selected_var == 'Índices de Perfusão':
            ...
            for i, col in enumerate(['r1_pi_value', 'r2_pi_value', 'r3_pi_value']):
                with cols[i]:
                    st.metric(label=f"Média {var_names[col]}", 
                             value=f"{df[col].mean():.2f}")
        
        elif selected_var == 'Pressões Arteriais':
            st.metric(label="Valores Faltantes Totais", 
                     value=df[['systolic_mmhg', 'diastolic_mmhg', 'map_mmhg']].isnull().sum().sum())
            
            cols = st.columns(3)
            for col in ['systolic_mmhg', 'diastolic_mmhg', 'map_mmhg']:
                with cols[['systolic_mmhg', 'diastolic_mmhg', 'map_mmhg'].index(col)]:
                    st.metric(label=f"Média {var_names[col]}", 
                             value=f"{df[col].mean():.2f}")
        
        else:
            original_var = [k for k, v in var_names.items() if v == selected_var][0]
            st.metric(label="Valores Faltantes", value=df[original_var].isnull().sum())
            st.metric(label="Valores Únicos", value=df[original_var].nunique())
            st.metric(label="Média", value=f"{df[original_var].mean():.2f}")
            st.metric(label="Mediana", value=f"{df[original_var].median():.2f}")
